...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
			...
			<a target="_blank">
				<xsl:attribute name="href">
					<xsl:text>http://www.everytrail.com/fullscreen.php?trip_id=</xsl:text><xsl:value-of
						select="@tripId"/>
				</xsl:attribute>Смотреть карту во весь экран<xsl:text/>
			</a>
		</center>
	</xsl:template>
	...
	<!-- Прежний шаблон "Вставляем карту c EveryTrail" -->
	<xsl:template name="insert.everytrail.old.not.used">
		<xsl:variable name="tripId"
			select="*[contains(@class,' topic/param ')][@name='EveryTrail']/@value"/>
		<center>
			<iframe marginheight="0" marginwidth="0" frameborder="0" scrolling="no">
				<xsl:attribute name="src">
					<xsl:text>http://www.everytrail.com/fullscreen.php?trip_id=</xsl:text>
					<xsl:value-of select="$tripId"/>
